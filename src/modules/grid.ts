...
class Grid {
  ...
  fill() {
    for (let i = 0; i < row * col; i++) {
      const ElementCtor = currentElement.constructor as unknown as {
        new (index: number): Element;
      };
  ...
  setBrush(x: number, y: number) {
    const elementClass = currentElement.constructor as unknown as any;
          ...
          if (this.isValidIndex(i, j)) {
            ...
            if (Math.random() < elementClass.currentProbability) {
              if (
                ...
                let newElement =
                  currentElement instanceof Empty
                    ? new Empty(j * this.col + i)
                    : new (currentElement.constructor as unknown as {
                        new (index: number): Element;
                      })(j * this.col + i);
  ...
  isPassable(i: number) {
    return (
      this.grid[i] instanceof Empty || this.grid[i].liquid || this.grid[i].gas
    );
  }
